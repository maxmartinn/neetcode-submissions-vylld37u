...
class MedianFinder:
    ...
    def addNum(self, num: int) -> None:
        ...
        if largeHeap and -smallHeap[0] > largeHeap[0]:
            smallVal = heapq.heappop(smallHeap)
            heapq.heappush(largeHeap, -smallVal)
        if len(largeHeap) + 2 == len(smallHeap):
            val = heapq.heappop(smallHeap)
            heapq.heappush(largeHeap, -val)
        if len(largeHeap) > len(smallHeap):
            largeVal = heapq.heappop(largeHeap)
            heapq.heappush(smallHeap, -largeVal)
